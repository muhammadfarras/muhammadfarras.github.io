...
Kekurangan model tersebut adalah tidak memperhatikan makna dari kata, hanya menfokuskan apakah nilai yang dicari mengandung kata tersebut atau tidak. Misalkan kalimat _Sunny Day_ dan _Bright Skies_ adalah makna yang sama, akan tetapi dengan model embedding ini kata tersebut dianggap tidak sama.
...
Kemampuan dalam melanjutkan kata ini berasal dari pemahaman dari makna kata (semantics, _understanding of meaning_), belajar untuk memahami bagaimana kata digunakan, bagaimana makna kata digabung dengan kata lainnya. Makna kata dirubah kedalam bentuk vektor menggunakan _model embedding_ sehingga dapat digunakan untuk beberapa tujuan tertentu.
...
!!! info 
    Banyak embedding model yang ada diluar, berikut saya lampirkan rangking embedding model yang tersedia pada HuggingFace.
...
!!! warning
    ...
    Ketika catatan bagian ini sudah hampir 70% saya selesaikan, ternyata ketika baca documentasi saat catatan ini dibuat (22 Desember 2025) dari [PGVector](https://github.com/langchain-ai/langchain-postgres?tab=readme-ov-file#vectorstore) lebih baik kita menggunakan `PGVectorStore` alih-alih `PGVector`. Maka dari itu beberapa lab saya buat as is, namun saya buat juga lab ke-2 yang menggunakan `PGVectorStore`. Jadi setelah bagian [Adding documents kedalam vector store](./03.%20RAG-1.md#a dding documents kedalam vector store) beralih ke lab yang menggunakan `PGVectorStore`.

    ![alt text](./assets/5_deprecate_pgvector.png)
...
!!! quote "Code"
    ...
    === "Check container"
        ...
        524be1c5c7ae   pgvector/pgvector:pg18-trixie   "docker-entrypoint.s…"   3 seconds ago   Up 2 seconds              0.0.0.0:6060->5432/tcp, [::]:6060->5432/tcp   pg_vector_farras
        ```
...
!!! quote "Code"
    ...
    === "Mencari kimiripan makna dari kata yang tidak ada pada document"
        ...
        0.6768340092253472
        ```
### Menyimpan Embedding pada vector store (menggunakan PGVectorStore _the new way_)
...
!!! info "Official Documentations"
    ...
    Alasan mengapa migrasi dan cara migrasi dari PGVector ke PGVectorStore [migrate_pgvector_to_pgvectorstore.ipynb](https://github.com/langchain-ai/langchain-postgres/blob/main/examples/migrate_pgvector_to_pgvectorstore.ipynb)
...
!!! notes "Lab"
    ...
    Untuk hands-on dapat melihat pada [PGVectorStore](./lab/6_documents_into_text_PGvectorStore.ipynb)
...
!!! quote "Code"
    ...
    === "Adding documents into vector store"
        ...
        Untuk hands-on dapat melihat pada lab [inisiasi object PGVectore Store](./lab/6_documents_into_text_PGvectorStore.ipynb)
        ...
        Untuk hands-on dapat melihat pada lab [store data ke vector store](./lab/6_documents_into_text_PGvectorStore.ipynb#store-data-ke-vector-store)
...
!!! quote "Code"
    ...
    === "Delete data pada vector store"

        Untuk hands-on dapat melihat pada lab [inisiasi object PGVectore Store](./lab/6_documents_into_text_PGvectorStore.ipynb#Delete-data-using-ID-index)
...
!!! quote "Code"
    ...
    === "Mencari kimiripan makna dari kata 'Semoga tahun ini adalah tahun terkahir di Vensys'"
...
[^4]: [Faster similarity search performance](https://cloud.google.com/blog/products/databases/faster-similarity-search-performance-with-pgvector-indexes) & [penjelasan HNSW](https://www.youtube.com/watch?v=QvKMwLjdK-s&t=54s)
...
!!! notes "Lab"
    ...
    Untuk hands-on dapat melihat pada [Tracking Change of your data](./lab/6_tracking_change_documents.ipynb)
...
* [Migration from pGVector to PGVectorStore](https://github.com/langchain-ai/langchain-postgres/blob/main/examples/migrate_pgvector_to_pgvectorstore.ipynb)
